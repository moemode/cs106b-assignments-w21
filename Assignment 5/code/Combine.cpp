...
/**
 * Merges two sorted vectors of DataPoint objects into a single sorted vector.
 *
 * This function takes two input vectors, `left` and `right`, both sorted in non-decreasing order
 * based on the `weight` attribute of the DataPoint objects. It merges them into a single vector
 * such that the resulting vector is also sorted in non-decreasing order by weight.
 *
 * @param left  The first sorted vector of DataPoint objects.
 * @param right The second sorted vector of DataPoint objects.
 * @return A new Vector<DataPoint> containing all elements from both input vectors, sorted by weight.
 */
Vector<DataPoint> myMerge(const Vector<DataPoint>& left, const Vector<DataPoint>& right) {
    Vector<DataPoint> merged;
    long l = 0, r = 0, lSize = left.size(), rSize = right.size();
    ...
    while (l < lSize) merged.add(left[l++]);
    while (r < rSize) merged.add(right[r++]);
    return merged;
}


/**
 * Recursively merges a list of sorted vectors of DataPoint objects into a single sorted vector.
 *
 * This function takes a vector of vectors, where each inner vector is sorted in non-decreasing
 * order based on the `weight` attribute of the DataPoint objects. It recursively combines these
 * vectors into a single sorted vector. The function uses a divide-and-conquer approach, splitting
 * the list of vectors into smaller sublists, merging them, and then combining the results.
 *
 * @param sequences A vector of sorted vectors of DataPoint objects.
 * @return A new Vector<DataPoint> containing all elements from the input vectors, sorted by weight.
 */
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    ...
    auto lhSorted = combine(sequences.subList(0, seqSize / 2));
    auto rhSorted = combine(sequences.subList(seqSize / 2));
    return myMerge(lhSorted, rhSorted);
}


/* * * * * * Test Cases Below This Point * * * * * */

STUDENT_TEST("Merges one sequence with a single element and one empty sequence") {
    DataPoint singleElement = { "Single Element", 42 };

    // Test merging with the single element sequence first
    auto merged = combine({{ singleElement }, {}});
    EXPECT_EQUAL(merged.size(), 1);
    EXPECT_EQUAL(merged[0], singleElement);

    // Test merging with the empty sequence first
    merged = combine({ {}, { singleElement } });
    EXPECT_EQUAL(merged.size(), 1);
    EXPECT_EQUAL(merged[0], singleElement);
}
