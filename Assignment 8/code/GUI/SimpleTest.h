...
namespace SimpleTest {
    namespace Internal {
        ...
        #define DO_ADD_TEST(fn, adder, name, type, file, line)\
            ...
            template <> void SimpleTest::Internal::testCase<JOIN(_testName, line), line>();\
            static SimpleTest::Internal::TestCaseAdder JOIN(adder, line)(file, line, name, type, SimpleTest::Internal::testCase<JOIN(_testName, line), line>);\
            template <> void SimpleTest::Internal::testCase<JOIN(_testName, line), line>()
        ...
        #undef EXPECT_EQUAL
        #define EXPECT_EQUAL(student, ...) DO_COMPOUND_EXPECT("EXPECT_EQUAL", "!=", areEqual, student, __VA_ARGS__)

        #undef EXPECT_NOT_EQUAL
        #define EXPECT_NOT_EQUAL(student, ...) DO_COMPOUND_EXPECT("EXPECT_NOT_EQUAL", "==", areNotEqual, student, __VA_ARGS__)
        ...
        #define DO_COMPOUND_EXPECT(name, failSymbol, comparator, student, ...)                      \
            do {                                                                                    \
               ...
               SimpleTest::Internal::doExpect(SimpleTest::Internal::comparator(_studentAnswer, _referenceAnswer), _expression.str(), __LINE__);\
            } while (0)
        ...
        #define ALLOW_TEST_ACCESS() template <const char*, int> friend void SimpleTest::Internal::testCase();
    }
}
