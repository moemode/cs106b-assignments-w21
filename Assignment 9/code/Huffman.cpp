...
EncodingTreeNode* huffmanTreeFor(const string& str) {
    ...
    if(charCount.size() < 2) {
        error("huffmanTreeFor assumes string has at least 2 distinct characters.");
    }
    for(const auto& [ch, chCount] : charCount) {
        trees.enqueue(new EncodingTreeNode{ch, nullptr, nullptr}, chCount);
    }
    while(trees.size() >= 2) {
        double zeroPrio = trees.peekPriority();
        EncodingTreeNode* zero = trees.dequeue();
        double onePrio = trees.peekPriority();
        EncodingTreeNode* one = trees.dequeue();
        EncodingTreeNode* combined = new EncodingTreeNode{0, zero, one};
        trees.enqueue(combined, zeroPrio + onePrio);
    }
    return trees.peek();
}
...
STUDENT_TEST("huffmanTreeFor solves q1.") {
    /* This tree:
     *                 *
     *                / \
     *               b   *
    ...
    EncodingTreeNode* reference = new EncodingTreeNode {
        '*',
        new EncodingTreeNode { 'b', nullptr, nullptr },  // Frequency 4
        new EncodingTreeNode { '*',
            new EncodingTreeNode { 'a', nullptr, nullptr }, // Frequency 2
            new EncodingTreeNode { 'c', nullptr, nullptr }  // Frequency 3
        }
    };
